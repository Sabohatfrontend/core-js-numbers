...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  return Math.acos((x1 * x2 + y1 * y2) / (u * v));
}
...
function toNumber(value, def) {
  if (Number.isNaN(Number.parseFloat(value))) return def;
  return value;
}
...
function getSumOfNumbers(x1, x2, x3) {
  const result = parseFloat(x1) + parseFloat(x2) + parseFloat(x3);
...
function getCountOfOddNumbers(number) {
  let num = 0;
  for (let i = 1; i <= Math.abs(number); i += 1) if (i % 2 === 1) num += 1;
  return num;
}
